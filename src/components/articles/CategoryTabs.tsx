...
const CategoryTabs = ({
  ...
  return <div>
      <div className="flex overflow-x-auto whitespace-nowrap px-2 border-t border-b border-gray-200 bg-gray-50 py-0 my-0">
        {sortedCategories.map(category => <Button key={category.id} onClick={() => setActiveCategory(category.id)} variant="ghost" className={cn("min-w-fit rounded-none border-b-2 mx-1 px-3", activeCategory === category.id ? "border-pana-red text-pana-red font-medium" : "border-transparent hover:border-gray-300")}>
            {category.name}
